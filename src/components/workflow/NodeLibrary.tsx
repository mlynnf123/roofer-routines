  ...
  return <Card className="w-80 shadow-lg border-border bg-card">
      <CardHeader className="pb-3 bg-white/80 backdrop-blur-sm rounded">
        ...
        <div className="space-y-3">
          ...
          <Button variant="outline" size="sm" className="w-full justify-start text-slate-600 bg-slate-100 text-sm font-light">
        ...
        <ScrollArea className="h-96">
          <div className="space-y-4">
            ...
            return <div key={category}>
                  ...
                  return <div key={template.id} className="flex items-center gap-3 p-2 rounded-md cursor-grab bg-white hover:bg-gray-50 border border-gray-200 transition-colors group" draggable onDragStart={e => {
                    e.dataTransfer.setData('application/reactflow', template.type);
                    e.dataTransfer.setData('application/json', JSON.stringify(template));
                    e.dataTransfer.effectAllowed = 'move';
                  }} onClick={() => onNodeSelect(template)}>
                          <div className={cn('w-6 h-6 rounded flex items-center justify-center flex-shrink-0 text-xs', template.color === 'trigger' && 'bg-blue-100 text-blue-700', template.color === 'action' && 'bg-green-100 text-green-700', template.color === 'condition' && 'bg-orange-100 text-orange-700', template.color === 'approved' && 'bg-purple-100 text-purple-700', template.color === 'step' && 'bg-gray-100 text-gray-700')}>
